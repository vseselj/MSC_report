...
\usepackage{tikz}	\usetikzlibrary{calc,patterns,decorations.pathmorphing,decorations.markings}
\usetikzlibrary{arrows, shapes, calc, positioning, chains}
...
\section{Rekurentne neuralne mrezhe}
...
itemc/.style={item,on chain,join}]
 ...
 \path node (A0) {$\mathbf{h}^0$} node (A1) {$\mathbf{h}^{1}$} node (A2) {$\mathbf{h}^{2}$} node[xshift=2em] (At)
 {$\mathbf{h}^{3}$};
 \end{scope}
...
\begin{align}
\mathbf{h}^t &= l_h(\mathbf{W_{hx}}\mathbf{x}^t+\mathbf{W_{hh}}\mathbf{h}^{t-1}+\mathbf{b_h})\\
\mathbf{y}^t &= l_y(\mathbf{W_{yh}}\mathbf{h}^t+\mathbf{b_y})
\end{align}
...
\begin{tikzpicture}[item/.style={circle,draw,thick,align=center}]
 \node[left=0,item, inner sep=2pt, minimum size=2cm] (AL) {$\mathbf{h}$};
 
 \draw[white,line width=0.8ex] (AL.north) -- ++ (0,1.9em);
 \draw[very thick,-latex] (AL.north) -- ++ (0,2em)
 node[above,item,fill=gray!10,inner sep=2pt, minimum size=2cm](y) {$\mathbf{y}^t$};
 \draw[very thick,latex-] (AL.south) -- ++ (0,-2em)
 node[below,item,fill=gray!10,inner sep=2pt, minimum size=2cm] {$\mathbf{x}^t$};
 
 \node[right=2, item, fill=gray!10,inner sep=2pt, minimum size=2cm](s) {$\mathbf{s}$};
 \draw[very thick,-latex] (s.west) to [out=-150,in=-30] (AL.east);
 \draw[very thick,-latex](y.east) to (s.north);
 \draw[very thick,-latex](s.east) to [out=0,in=-90,distance=3cm](s.south);
\end{tikzpicture}
